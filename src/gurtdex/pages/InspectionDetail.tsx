...
export function InspectionDetail() {
  ...
  const state = useAsync<Data>(async () => {
    ...
    const [site, items, defects] = await Promise.all([
      fetchSite(client, inspection.site_id),
      fetchItems(client, inspection.site_id),
      fetchDefects(client, { inspectionId: inspection.id }),
    ]);
    const item =
      items.find((e) => e.id === inspection.item_id) ?? null;
    return { inspection, site, item, defects };
  }, [client, id]);

  return (
    <LoadGuard state={state}>
      {({ inspection: i, site, item, defects }) => (
        <>
          ...
          <p className="muted">
            {item ? <b>{item.name}</b> : 'PSA-Artikel unbekannt'}
            {' · '}
            {site ? (
          ...
          <div className="kpi-grid">
            <div className="card">
              <div className="kpi-label">PSA-Artikel</div>
          ...
          <div className="section-head">
            <h2>Checkliste (DGUV G 312-906 / EN 365)</h2>
          </div>
