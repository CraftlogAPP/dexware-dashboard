...
const INSPECTION_COLS =
  'id, org_id, site_id, item_id, type, started_at, lat, lng, gps_accuracy_m, checklist, notes, performed_by, inspector_name, canceled, cancel_reason, canceled_at, created_at';
const DEFECT_COLS =
  'id, org_id, site_id, inspection_id, item_id, title, description, severity, item_blocked, status, reported_by, reporter_name, resolved_at, resolved_by, resolver_name, resolution_note, created_at';
const ITEM_COLS =
  'id, org_id, site_id, name, category, manufacturer, model, serial_no, manufacture_date, first_use_date, max_life_years, wearer_name, notes, retired, created_at';
...
/** PSA-Inventar (ohne Referenzfotos), optional je Standort. */
export async function fetchItems(
  sb: SupabaseClient,
  siteId?: string,
): Promise<PsaItem[]> {
  let q = sb.from('psa_item').select(ITEM_COLS).order('retired').order('name');
  if (siteId) q = q.eq('site_id', siteId);
  const { data, error } = await q;
  if (error) fail('PSA-Artikel konnten nicht geladen werden', error);
  return (data ?? []) as PsaItem[];
}
...
/** PSA-Artikel anlegen/ändern — Upsert wie app-seitiges saveItem (photo_url bleibt unangetastet). */
export async function saveItem(
  ...
  const { error } = await sb.from('psa_item').upsert({
    id: existing?.id ?? crypto.randomUUID(),
    org_id: orgId,
    ...input,
    created_at: existing?.created_at ?? new Date().toISOString(),
  });
  if (error) fail('PSA-Artikel konnte nicht gespeichert werden', error);
}
...
export async function addDefect(
  ...
  const { error } = await sb.from('defect').insert({
    ...
    item_blocked: input.item_blocked,
    photo_urls: [],
    reported_by: userId,
...
export async function fetchInspectionMeta(
  ...
  const { data, error } = await sb
    .from('inspection')
    .select('id, site_id, item_id, type, started_at, canceled')
    .order('started_at', { ascending: false })
