...
const INSPECTION_COLS =
  'id, org_id, site_id, ladder_id, type, started_at, lat, lng, gps_accuracy_m, checklist, notes, performed_by, inspector_name, canceled, cancel_reason, canceled_at, created_at';
const DEFECT_COLS =
  'id, org_id, site_id, inspection_id, ladder_id, title, description, severity, ladder_blocked, status, reported_by, reporter_name, resolved_at, resolved_by, resolver_name, resolution_note, created_at';
const LADDER_COLS =
  'id, org_id, site_id, name, category, material, manufacturer, purchase_year, length_m, rung_count, notes, retired, created_at';
...
/** Leitern-/Tritte-Inventar (ohne Referenzfotos), optional je Standort. */
export async function fetchLadders(
  sb: SupabaseClient,
  siteId?: string,
): Promise<Ladder[]> {
  let q = sb.from('ladder').select(LADDER_COLS).order('retired').order('name');
  if (siteId) q = q.eq('site_id', siteId);
  const { data, error } = await q;
  if (error) fail('Leitern konnten nicht geladen werden', error);
...
/** Leiter/Tritt anlegen/ändern — Upsert wie app-seitiges saveLadder (photo_url bleibt unangetastet). */
export async function saveLadder(
  ...
  const { error } = await sb.from('ladder').upsert({
    id: existing?.id ?? crypto.randomUUID(),
    org_id: orgId,
    ...input,
    created_at: existing?.created_at ?? new Date().toISOString(),
  });
  if (error) fail('Leiter konnte nicht gespeichert werden', error);
}
...
export async function addDefect(
  ...
  const { error } = await sb.from('defect').insert({
    ...
    ladder_blocked: input.ladder_blocked,
    photo_urls: [],
    reported_by: userId,
...
export async function fetchInspections(
  ...
  if (filter.ladderId) q = q.eq('ladder_id', filter.ladderId);
...
export async function fetchInspectionMeta(
  ...
  const { data, error } = await sb
    .from('inspection')
    .select('id, site_id, ladder_id, type, started_at, canceled')
    .order('started_at', { ascending: false })
