...
export function InspectionDetail() {
  ...
  const state = useAsync<Data>(async () => {
    ...
    const [site, ladders, defects] = await Promise.all([
      fetchSite(client, inspection.site_id),
      fetchLadders(client, inspection.site_id),
      fetchDefects(client, { inspectionId: inspection.id }),
    ]);
    const ladder = ladders.find((l) => l.id === inspection.ladder_id) ?? null;
    return { inspection, site, ladder, defects };
  }, [client, id]);

  return (
    <LoadGuard state={state}>
      {({ inspection: i, site, ladder, defects }) => (
        <>
          ...
          <p className="muted">
            {ladder ? <b>{ladder.name}</b> : 'Leiter unbekannt'}
            {' · '}
            {site ? (
              <Link to={`../standorte/${site.id}`}>{site.name}</Link>
            ) : (
              'Standort unbekannt'
            )}
            {site ? ` · ${site.address}` : ''}
          ...
          <div className="kpi-grid">
            <div className="card">
              <div className="kpi-label">Leiter / Tritt</div>
              <div>{ladder ? ladder.name : '—'}</div>
              <div className="muted small">
                {ladder ? `${ladder.category} · ${ladder.material}` : ''}
              </div>
            </div>
            <div className="card">
          ...
          <div className="section-head">
            <h2>Checkliste (DGUV Information 208-016)</h2>
          </div>
          {Object.keys(i.checklist).length === 0 ? (
            <div className="card empty">
              Keine Checkliste erfasst (z. B. regelmäßige Prüfung — nur Termin und
