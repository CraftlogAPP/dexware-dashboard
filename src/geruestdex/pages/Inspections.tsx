...
import { addInspection, cancelInspection, fetchInspections, fetchSites } from '../api';
import { InspectionBadge } from '../badges';
import { checklistSummary, siteNameMap } from '../labels';
import {
  CHECKLIST,
  INSPECTION_LABELS,
  type CheckResult,
...
export function Inspections() {
  ...
  // Stammdaten (Baustellen) ändern sich durch Filter nicht — einmal laden.
  const baseState = useAsync<Site[]>(() => fetchSites(client), [client]);
  ...
  async function onAdd(v: FormValues) {
    ...
    const site = baseState.data?.find((st) => st.id === s(v.site_id));
  ...
  return (
    <>
      ...
      <p className="muted" style={{ marginTop: -6 }}>
        Append-only-Prüfprotokoll je Baustelle — jeder Eintrag bleibt unveränderlich,
          ...
          fields={[
            {
              ...
              kind: 'select',
              required: true,
              hint: 'Die Prüfung wird der Baustelle zugeordnet (Gerüst-Inventar siehe Baustellen-Detail).',
              options: sites
                .filter((st) => st.active)
