...
const INSPECTION_COLS =
  'id, org_id, site_id, gate_id, type, started_at, lat, lng, gps_accuracy_m, checklist, notes, performed_by, inspector_name, canceled, cancel_reason, canceled_at, created_at';
const DEFECT_COLS =
  'id, org_id, site_id, inspection_id, gate_id, title, description, severity, gate_blocked, status, reported_by, reporter_name, resolved_at, resolved_by, resolver_name, resolution_note, created_at';
const GATE_COLS =
  'id, org_id, site_id, name, category, drive_type, manufacturer, build_year, dimensions, serial_no, notes, retired, created_at';

/** Einzelnes Objekt per Primärschlüssel. */
...
/** Tor-Inventar (ohne Referenzfotos), optional je Objekt. */
export async function fetchGates(
  sb: SupabaseClient,
  siteId?: string,
): Promise<Gate[]> {
  let q = sb.from('gate').select(GATE_COLS).order('retired').order('name');
  if (siteId) q = q.eq('site_id', siteId);
  const { data, error } = await q;
  if (error) fail('Tore konnten nicht geladen werden', error);
  return (data ?? []) as Gate[];
}
...
/** Objekt anlegen/ändern — Upsert wie app-seitiges saveSite. */
...
/** Tor anlegen/ändern — Upsert wie app-seitiges saveGate (photo_url bleibt unangetastet). */
export async function saveGate(
  ...
  const { error } = await sb.from('gate').upsert({
    id: existing?.id ?? crypto.randomUUID(),
    org_id: orgId,
    ...input,
    created_at: existing?.created_at ?? new Date().toISOString(),
  });
  if (error) fail('Tor konnte nicht gespeichert werden', error);
}
...
export async function addDefect(
  ...
  const { error } = await sb.from('defect').insert({
    ...
    gate_blocked: input.gate_blocked,
    photo_urls: [],
    reported_by: userId,
...
export async function fetchInspections(
  ...
  if (filter.gateId) q = q.eq('gate_id', filter.gateId);
...
export async function fetchInspectionMeta(
  ...
  const { data, error } = await sb
    .from('inspection')
    .select('id, site_id, gate_id, type, started_at, canceled')
    .order('started_at', { ascending: false })
...
/** Prüfungen eines Objekts im Zeitraum inkl. Fotos — nur für den PDF-Bericht. */
...
/** Mängel eines Objekts im Zeitraum inkl. Fotos — nur für den PDF-Bericht. */
