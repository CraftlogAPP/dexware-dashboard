// Prüfprotokoll Tore — HTML-Aufbau nach dem Muster von mobile/src/pdf/report.ts,
...
import { checklistSummary, gateTypeLabel, buildLabel } from './labels';
import {
  INSPECTION_LABELS,
  LEGAL_BASIS,
  SEVERITY_LABELS,
...
function gateName(id: string | null, gates: Gate[]): string {
  if (!id) return 'Objekt allgemein';
  return gates.find((e) => e.id === id)?.name ?? 'Tor';
}

function inspectionRow(i: InspectionWithPhotos, gates: Gate[]): string {
  ...
  return `<tr class="${i.canceled ? 'canceled' : ''}">
    ...
    <td>${esc(gateName(i.gate_id, gates))}</td>
...
function defectRow(d: DefectWithPhotos, gates: Gate[]): string {
  const resolution =
    d.status === 'resolved'
  ...
  return `<tr>
    ...
    <td>${esc(gateName(d.gate_id, gates))}${d.gate_blocked ? '<div class="storno">Stillgelegt — gegen Benutzung gesichert</div>' : ''}</td>
...
export function buildReportHtml(
  ...
  const rows = inspections.map((i) => inspectionRow(i, gates)).join('');
  const active = inspections.filter((i) => !i.canceled).length;
  const openCount = defects.filter((d) => d.status === 'open').length;

  const activeGates = gates.filter((e) => !e.retired);
  const inventory =
    activeGates.length > 0
      ? `<div class="meta"><b>Tor-Inventar:</b> ${activeGates
          .map(
            (e) =>
              `${esc(e.name)} (${esc(gateTypeLabel(e.category, e.drive_type))}${
                e.build_year || e.dimensions ? `, ${esc(buildLabel(e.build_year, e.dimensions))}` : ''
              }${e.serial_no ? `, Nr. ${esc(e.serial_no)}` : ''})`,
          )
          .join(' · ')}</div>`
      : '';
  ...
  <table>
    <thead><tr>
      <th>Gemeldet</th><th>Mangel / Instandsetzung</th><th>Tor</th><th>Einstufung (Ampel)</th><th>Gemeldet von</th>
    </tr></thead>
    <tbody>${defects.map((d) => defectRow(d, gates)).join('')}</tbody>
  </table>`
      : '';
...
<head>
<meta charset="utf-8">
<title>Prüfprotokoll Tore — ${esc(site.name)}</title>
<style>
  ...
  .legal { margin: 10px 0 18px; padding: 8px 12px; background: #f3effb; border-left: 3px solid #7C3AED; font-size: 11px; }
...
<body>
  <h1>Prüfprotokoll Tore</h1>
  <div class="meta"><b>${esc(org.name)}</b>${org.address ? ` · ${esc(org.address)}` : ''}</div>
  <div class="meta">Objekt: <b>${esc(site.name)}</b> · ${esc(site.address)}</div>
  ${
    site.operator_name
  ...
  <table>
    <thead>
      <tr>
        ...
        <th>Tor</th>
        <th>Prüfart</th>
        <th>Ergebnis (Checkliste ASR A1.7 / DGUV I 208-022)</th>
        <th>GPS-Stempel</th>
        <th>Geprüft von</th>
        <th>Fotos</th>
      </tr>
  ...
  <div class="footer">
    <span>Einträge sind append-only dokumentiert; Korrekturen nur als gekennzeichnete Stornos. Erstellt mit ToreDex — toredex.dexware.app</span>
