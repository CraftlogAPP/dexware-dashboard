...
export function Overview() {
  ...
  const state = useAsync<OverviewData>(async () => {
    const [sites, extinguishers, meta, recentInspections, openDefects] =
      await Promise.all([
        fetchSites(client),
    ...
    return { sites, extinguishers, meta, recentInspections, openDefects };
  }, [client]);
    ...
    <>
      ...
      {orgCtx && (
        <p className="muted" style={{ marginTop: -6 }}>
          {orgCtx.org.name} · Prüfpflichten im Blick — live aus der FeuerDex-App.
        </p>
      )}

      <LoadGuard state={state}>
        {({ sites, extinguishers, meta, recentInspections, openDefects }) => {
            ...
            <>
              <div className="kpi-grid">
                ...
                <div className="kpi">
                  ...
                  <div className="kpi-value">{dueExtinguishers.length}</div>
                  <div className="kpi-sub">
                    von {activeExtinguishers.length} aktiven Löschern
                  </div>
                </div>
              </div>

              {dueExtinguishers.length > 0 && (
                <>
                  ...
                  <div className="table-wrap">
                    <table>
                      ...
                      <tbody>
                        {dueExtinguishers.slice(0, 8).map((e) => (
                          <tr key={e.id}>
                            <td>
