...
export function InspectionDetail() {
  ...
  const state = useAsync<Data>(async () => {
    ...
    const [site, extinguishers, defects] = await Promise.all([
      fetchSite(client, inspection.site_id),
      fetchExtinguishers(client, inspection.site_id),
      fetchDefects(client, { inspectionId: inspection.id }),
    ]);
    const extinguisher =
      extinguishers.find((e) => e.id === inspection.extinguisher_id) ?? null;
    return { inspection, site, extinguisher, defects };
  }, [client, id]);

  return (
    <LoadGuard state={state}>
      {({ inspection: i, site, extinguisher, defects }) => (
        <>
          ...
          <p className="muted">
            {extinguisher ? <b>{extinguisher.name}</b> : 'Feuerlöscher unbekannt'}
            {' · '}
            {site ? (
          ...
          <div className="kpi-grid">
            <div className="card">
              <div className="kpi-label">Feuerlöscher</div>
              <div>{extinguisher ? extinguisher.name : '—'}</div>
              <div className="muted small">
                {extinguisher
                  ? `${extinguisher.category} · ${extinguisher.build_type}`
                  : ''}
              </div>
            </div>
            <div className="card">
          ...
          <div className="section-head">
            <h2>Checkliste (ASR A2.2 / DIN 14406-4)</h2>
          </div>
